classdef PLL_Class
    ...
    methods (Access = public)
        ...
        function obj = runPLL(obj, input)
            % clear oscilator phase state because we are operating on
            % samples that have already been corrected with the current
            % PLL state
            theta = 0;
            obj.oscilator_phase_prev = 0;
            
            for idx = 1:length(input)
                % get phase corrected input sample
                x = input(idx);
                s = x*exp(i*theta);
                ...
                % use numerically controlled oscilator to generate phase
                % correction
                [oscilator_phase, obj] = nco(obj, (freq - obj.curr_frequency));
                theta = -oscilator_phase;
            end
            
            % save frequency and next symbol phase
